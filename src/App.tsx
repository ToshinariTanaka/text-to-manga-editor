...
function App() {
  ...
  return <main className="app"><h1>文章→漫画エディタ (MVP)</h1><SourceInput sourceText={sourceText} onChange={setSourceText} /><SettingsPanel settings={settings} onChange={setSettings} /><CharacterManager characters={characters} onChange={setCharacters} /><section className="card"><h2>6. 漫画設計図作成</h2><button onClick={onGenerate}>漫画設計図を作成</button></section><StoryboardEditor storyboard={storyboard} characters={characters} textLayers={textLayers} onChange={setStoryboard} onTextLayersChange={setTextLayers} /><TextLayerEditor textLayers={textLayers} onChange={setTextLayers} /><MangaPreview storyboard={storyboard} textLayers={textLayers} showPanelNumbers={showPanelNumbers} previewRef={previewRef} /><ExportPanel showPanelNumbers={showPanelNumbers} setShowPanelNumbers={setShowPanelNumbers} onExport={onExport} onSave={() => saveAppState(appState)} onLoad={() => { const loaded = loadAppState(); if (loaded) { setSourceText(loaded.sourceText); setSettings(loaded.settings); setCharacters(loaded.characters.map((character) => ({ ...character, useInManga: character.useInManga ?? character.id === 'leady_default', appearanceNotes: character.appearanceNotes ?? '', referenceImages: character.referenceImages ?? [] }))); setStoryboard(loaded.storyboard); setTextLayers(loaded.textLayers); setShowPanelNumbers(loaded.showPanelNumbers); } }} onReset={() => { resetAppState(); setSourceText(sampleText); setSettings(defaultSettings); setCharacters(defaultCharacters); setStoryboard(null); setTextLayers([]); setShowPanelNumbers(true); }} /></main>;
}
